...
// 센터를 실제로 삭제(DELETE)하지 않고 숨김 처리한다 — 이미 있는 center_config.is_active 컬럼을 재사용
// (기존 center-update 액션이 이미 is_active를 받아 UPDATE해주므로 백엔드 수정이 필요 없다).
// 실적/TO/KPI 등 데이터는 DB에 그대로 남고, 사이드바·전체현황 화면에서만 안 보이게 된다.
// 숨긴 동안에는 그 센터의 업로드 링크(토큰)와 미업로드 알림도 함께 멈춘다(index.ts의 is_active 검사를 그대로 공유하기 때문) —
// "당분간 안 쓰는 센터를 잠시 치워둔다"는 의도와 자연스럽게 맞아떨어짐. "숨긴 센터 관리"에서 언제든 다시 보이게 할 수 있음.
function hideCenterPrompt(code) {
  const meta = allCentersMeta.find(function(c) { return c.center_code === code; });
  if (!confirm((meta ? meta.center_name : code) + ' 센터를 숨기시겠습니까?\n등록된 실적 데이터는 DB에 그대로 남고, 목록·전체현황에서만 안 보이게 됩니다(숨긴 동안 업로드 링크와 미업로드 알림도 함께 멈춥니다).\n"⚙ 계정 ▾ → 🙈 숨긴 센터 관리"에서 언제든 다시 보이게 할 수 있습니다.')) return;
  fetch(SB_FUNCTION_URL + '?action=center-update', {
    method: 'POST', headers: { 'Authorization': 'Bearer ' + SB_ANON_KEY, 'Content-Type': 'application/json' },
    body: JSON.stringify({ workspace_password: workspacePasswordCache, center_code: code, is_active: false })
  }).then(function(r) { return r.json(); }).then(async function(data) {
...
function unhideCenter(code) {
  fetch(SB_FUNCTION_URL + '?action=center-update', {
    method: 'POST', headers: { 'Authorization': 'Bearer ' + SB_ANON_KEY, 'Content-Type': 'application/json' },
    body: JSON.stringify({ workspace_password: workspacePasswordCache, center_code: code, is_active: true })
  }).then(function(r) { return r.json(); }).then(async function(data) {
...
function renderHiddenCentersPanel() {
  ...
  if (!el) return;
  const hidden = allCentersMeta.filter(function(c) { return c.is_active === false; });
...
async function init() {
  ...
  await Promise.all([
    ...
    workspaceUnlocked ? loadWorkspaceIssuesAndMessages() : Promise.resolve()
  ]);
...
function renderSidebar() {
  ...
  list.innerHTML = overviewHtml + archiveHtml + issuesManagementHtml + notifyHtml + sorted.map(function(c, idx) {
    ...
    return '<div class="center-item ' + activeCls + '" onclick="selectCenter(\'' + c.center_code + '\')" title="' + c.center_name + ' · ' + signal.label + '">'
      + '<span class="center-name-text">' + signalDot + '🏢 ' + lockIcon + c.center_name + extraBadges + '</span>'
      + adminControls
      + '</div>'
...
async function loadWorkspaceIssuesAndMessages() {
  if (!workspaceUnlocked) { workspaceIssuesCache = []; workspaceMessagesSummary = {}; return; }
  try {
    const ts = Date.now();
    const [issuesData, msgData] = await Promise.all([
      fetch(SB_FUNCTION_URL + '?action=list-all-issues&workspace_password=' + encodeURIComponent(workspacePasswordCache) + '&_ts=' + ts, { headers: { 'Authorization': 'Bearer ' + SB_ANON_KEY }, cache: 'no-store' }).then(function(r) { return r.json(); }),
      fetch(SB_FUNCTION_URL + '?action=list-all-messages-summary&workspace_password=' + encodeURIComponent(workspacePasswordCache) + '&_ts=' + ts, { headers: { 'Authorization': 'Bearer ' + SB_ANON_KEY }, cache: 'no-store' }).then(function(r) { return r.json(); })
    ]);
    workspaceIssuesCache = issuesData.success ? (issuesData.issues || []) : [];
    workspaceMessagesSummary = msgData.success ? (msgData.summary || {}) : {};
  } catch (e) { workspaceIssuesCache = []; workspaceMessagesSummary = {}; }
}

// 특정 센터의 "확인 안 된 이슈" 건수 (사이드바 신호등용)
function getCenterUnreviewedIssueCount(centerCode) {
  return workspaceIssuesCache.filter(function(i) { return i.center_code === centerCode && !i.reviewed; }).length;
}

async function setIssueReviewed(id, reviewed) {
  try {
    const res = await fetch(SB_FUNCTION_URL + '?action=issues-mark-reviewed', {
      method: 'POST', headers: { 'Authorization': 'Bearer ' + SB_ANON_KEY, 'Content-Type': 'application/json' },
      body: JSON.stringify({ workspace_password: workspacePasswordCache, id: id, reviewed: reviewed })
    });
    const data = await res.json();
    if (!data.success) throw new Error(data.error || '처리 실패');
    const item1 = workspaceIssuesCache.find(function(i) { return i.id === id; });
...
async function loadAllCentersOverview() {
  try {
    ...
    const data = await res.json();
    if (data.success) {
      // 숨긴(is_active=false) 센터의 데이터는 전체현황에서 보이지 않아야 하므로 클라이언트에서 한 번 더 걸러낸다
      // (admin-overview 자체는 is_active를 걸러내지 않고 center_daily_performance를 그대로 반환하기 때문 —
...
function buildMatrix(records) {
  ...
  // 관리자에게만: 이 날짜에 등록된 이슈가 있으면 표에 작은 마커를 붙여준다(클릭하면 이슈 화면으로 이동).
  // workspaceIssuesCache가 관리자 로그인 때만 채워지므로, 센터장 화면에는 자동으로 아무것도 안 붙는다.
  const issueDatesForCenter = workspaceUnlocked
    ? new Set(workspaceIssuesCache.filter(function(i) { return i.center_code === currentCenter; }).map(function(i) { return i.issue_date; }))
    : new Set();

  const bodyHtml = records.slice().reverse().map(function(r) {
    const issueMark = issueDatesForCenter.has(r.report_date)
      ? '<span title="이 날짜에 등록된 이슈가 있습니다 (클릭하면 이동)" style="cursor:pointer;color:#FF6B70;margin-left:4px;" onclick="event.stopPropagation();jumpToIssueForDate(\'' + r.report_date + '\')">📝</span>'
      : '';
    return '<tr><td style="position:sticky;left:0;background:#1d1d1f;font-weight:600;">' + r.report_date + (isWeekendOrHoliday(r.report_date) ? ' 🔸' : '') + issueMark + '</td>'
...
function filterIssueList() {
  ...
  const filtered = !keyword ? allIssuesCache : allIssuesCache.filter(function(item) {
    ...
    const inDate = (item.issue_date || '').includes(keyword);
    return inTitle || inContent || inDate;
  });
  renderIssueList(filtered, keyword);
}

// 데이터 표(buildMatrix)의 이슈 마커를 클릭했을 때 - 이슈 화면으로 이동해 그 날짜로 검색해서 바로 보여준다.
async function jumpToIssueForDate(date) {
  currentMainTab = 'issues';
  renderSidebar();
  await renderIssues();
  const searchEl = document.getElementById('issueSearch');
  if (searchEl) { searchEl.value = date; filterIssueList(); }
}
...
// ============================================
let centerMessagesCache = [];

async function renderMessages() {
  const main = document.getElementById('main');
  if (!currentCenter) { main.innerHTML = '<div class="empty">센터를 선택해 주세요.</div>'; return; }
  const token = centerTokenMap[currentCenter];
  if (!workspaceUnlocked && !token) { main.innerHTML = '<div class="empty">이 센터는 아직 토큰이 등록되지 않았습니다.</div>'; return; }

  const centerInfo = allCenters.find(function(c) { return c.center_code === currentCenter; });
  const introText = workspaceUnlocked
    ? '이 센터에 메모를 보내고, 답변을 확인할 수 있습니다.'
    : '관리자가 보낸 메모를 확인하고 답변을 남길 수 있습니다.';

  main.innerHTML = '<div class="panel" style="max-width:640px;">'
    + '<h3>' + (centerInfo ? centerInfo.center_name : '') + ' · 관리자 메모(쪽지)</h3>'
    + '<p style="font-size:13px;color:#a1a1a6;margin:10px 0 6px;">' + introText + '</p>'
    + '<div id="messageThread" style="max-height:420px;overflow-y:auto;margin-bottom:12px;padding:4px;border:1px solid #2c2c2e;border-radius:8px;"><div class="loading">불러오는 중...</div></div>'
    + '<textarea id="messageInput" class="paste-box" style="min-height:70px;" placeholder="내용을 입력하세요"></textarea>'
    + '<button class="btn-primary" onclick="sendCenterMessage()">보내기</button>'
...
async function loadMessageThread() {
  ...
  const token = centerTokenMap[currentCenter];
  try {
    const params = workspaceUnlocked
      ? 'workspace_password=' + encodeURIComponent(workspacePasswordCache) + '&center_code=' + encodeURIComponent(currentCenter)
      : 'token=' + encodeURIComponent(token);
    const res = await fetch(SB_FUNCTION_URL + '?action=messages-list&' + params + '&_ts=' + Date.now(), { headers: { 'Authorization': 'Bearer ' + SB_ANON_KEY }, cache: 'no-store' });
    const data = await res.json();
    if (!data.success) throw new Error(data.error || '조회 실패');
    centerMessagesCache = data.messages || [];
    renderMessageThread();
    markMessagesRead();
  } catch (e) {
    if (threadEl) threadEl.innerHTML = '<div class="empty">불러오기 실패: ' + e.message + '</div>';
  }
}
...
function renderMessageThread() {
  const threadEl = document.getElementById('messageThread');
  if (!threadEl) return;
  if (centerMessagesCache.length === 0) { threadEl.innerHTML = '<div class="empty">주고받은 메모가 없습니다.</div>'; return; }
  threadEl.innerHTML = centerMessagesCache.map(function(m) {
    const isAdmin = m.sender === 'admin';
    const label = isAdmin ? '관리자' : '센터';
    const align = isAdmin ? 'flex-start' : 'flex-end';
    const bg = isAdmin ? 'rgba(90,200,250,.12)' : 'rgba(52,199,89,.12)';
    const when = new Date(m.created_at).toLocaleString('ko-KR');
    return '<div style="display:flex;flex-direction:column;align-items:' + align + ';margin-bottom:10px;">'
      + '<div style="font-size:11px;color:#86868b;margin-bottom:2px;">' + label + ' · ' + when + '</div>'
      + '<div style="max-width:80%;background:' + bg + ';border-radius:10px;padding:8px 12px;font-size:13px;white-space:pre-wrap;">' + escapeHtml(m.message) + '</div>'
      + '</div>';
  }).join('');
  threadEl.scrollTop = threadEl.scrollHeight;
}

async function sendCenterMessage() {
  ...
  try {
    const payload = workspaceUnlocked
      ? { workspace_password: workspacePasswordCache, center_code: currentCenter, message: text }
      : { token: token, center_code: currentCenter, message: text };
    const res = await fetch(SB_FUNCTION_URL + '?action=messages-send', {
      method: 'POST', headers: { 'Authorization': 'Bearer ' + SB_ANON_KEY, 'Content-Type': 'application/json' },
      body: JSON.stringify(payload)
    });
    const data = await res.json();
    ...
    await loadMessageThread();
    if (workspaceUnlocked) { await loadWorkspaceIssuesAndMessages(); renderSidebar(); }
  } catch (e) {
    statusEl.className = 'status-msg err';
...
// 상대방이 보낸 메시지를 "내가 읽었음"으로 표시(스레드를 열 때 자동 호출). 실패해도 조용히 무시(다음에 다시 열면 재시도됨).
async function markMessagesRead() {
  const token = centerTokenMap[currentCenter];
  try {
    const payload = workspaceUnlocked
      ? { workspace_password: workspacePasswordCache, center_code: currentCenter }
      : { token: token, center_code: currentCenter };
    await fetch(SB_FUNCTION_URL + '?action=messages-mark-read', {
      method: 'POST', headers: { 'Authorization': 'Bearer ' + SB_ANON_KEY, 'Content-Type': 'application/json' },
      body: JSON.stringify(payload)
    });
    if (workspaceUnlocked && workspaceMessagesSummary[currentCenter]) {
      workspaceMessagesSummary[currentCenter].unreadByAdmin = 0;
      renderSidebar();
    }
  } catch (e) { /* 읽음처리 실패는 조용히 무시 */ }
}
...
function renderNotificationSettings() {
  ...
  main.innerHTML = '<div style="--dash-accent:#FE2E36;--dash-accent-dark:#3a1518;">'
    ...
    + '<p style="font-size:12px;color:#86868b;margin:0 0 10px;">발송 정지/시각/반복주기만 전체 센터 공통이고, <b>발송 시점(며칠째)·제목·본문은 지금 선택된 "' + (notifSelectedCenterName || notifSelectedCenter || '') + '" 센터에만 저장</b>됩니다(센터마다 다르게 가능). 제목/본문에 {center_name}, {days}, {site_link}를 넣으면 발송 시 자동으로 치환됩니다.</p>'
    ...
    + '<h4 style="margin:18px 0 6px;font-size:13px;color:#f5a623;">📊 실적 미업로드 알림 (' + (s.warn_send_on_day || 4) + '일째 발송) · "' + (notifSelectedCenterName || notifSelectedCenter || '') + '" 전용 설정</h4>'
    ...
    + '<h4 style="margin:18px 0 6px;font-size:13px;color:#5ac8fa;">📝 이슈/히스토리 미등록 알림 (' + (s.issue_send_on_day || 4) + '일째 발송) · "' + (notifSelectedCenterName || notifSelectedCenter || '') + '" 전용 설정</h4>'
    ...
    + (s.has_center_override
        ? '<div style="font-size:11px;color:#5ac8fa;margin-top:6px;">✓ 이 센터는 발송 시점·문구를 전용으로 사용 중입니다. <button style="border:none;background:none;color:#86868b;text-decoration:underline;cursor:pointer;font-size:11px;padding:0;" onclick="resetCenterNotificationOverride()">전체 공통 기본값으로 되돌리기</button></div>'
        : '<div style="font-size:11px;color:#86868b;margin-top:6px;">현재 전체 공통 기본 발송 시점·문구를 쓰고 있습니다. 위 내용을 고쳐서 저장하면 이 센터에만 적용됩니다.</div>')
...
async function saveNotificationSettings() {
  ...
  try {
    ...
    if (!data.success) { statusEl.className = 'status-msg err'; statusEl.textContent = '저장 실패: ' + data.error; return; }
    statusEl.className = 'status-msg ok';
    statusEl.textContent = '저장되었습니다(발송 시점·문구는 "' + (allCentersMeta.find(function(c) { return c.center_code === notifSelectedCenter; }) || {}).center_name + '" 센터 전용). 다음 발송부터 이 설정이 적용됩니다.';
    await loadNotificationData();
    renderNotificationSettings();
  } catch (e) { statusEl.className = 'status-msg err'; statusEl.textContent = '오류: ' + e.message; }
}

// 지금 선택된 센터의 전용 발송 시점·문구를 삭제하고, 다시 전체 공통 기본값을 쓰도록 되돌린다.
async function resetCenterNotificationOverride() {
  ...
  if (!confirm('"' + (targetMeta ? targetMeta.center_name : notifSelectedCenter) + '" 센터의 전용 발송 시점·문구를 지우고 전체 공통 기본값으로 되돌리시겠습니까?')) return;
  const statusEl = document.getElementById('notifSettingsStatus');
  statusEl.className = 'status-msg';
...
function renderInputGuide() {
  // 평택시청은 텍스트 붙여넣기 방식을 쓰지 않으므로(항목별 입력폼만 사용) "사용설명보기" 자체가 필요 없음
  if (currentCenter === 'pyeongtaek') return '';
...
async function extractDailyReportAndFill() {
  ...
  // 통합 표: 추출된 날짜 범위로 표를 만들고(범위 안의 빈 날짜는 이월/공란), 추출된 값만 해당 날짜 칸에 채운다
  ...
  results.forEach(function(r) {
    ...
    Object.keys(combined).forEach(function(key) {
      ...
      const inp = document.querySelector('.pt-unified-input[data-row="' + ri + '"][data-key="' + key + '"]');
      if (inp) inp.value = combined[key];
    });
  });
...
// 통합 표를 저장하고, 성공하면 이번에 업로드했던 원본 hwpx 파일들을 업로드 자료함에도 반영한다.
async function savePyeongtaekDailyReportAll() {
  await savePtUnifiedRows();
...
async function renderEntry() {
  ...
  const dayReportBox = DAY_REPORT_CENTERS[currentCenter] ? renderPyeongtaekEntryPanel() : '';
  ...
  // 평택시청은 일일업무보고 자동추출 + 직접입력(항목별 입력폼) + 업무유형별 현황을 한 화면(탭 1개)으로 통합했으므로 별도 구성한다.
  const methods = [];
  const isPyeongtaek = currentCenter === 'pyeongtaek';
  if (isPyeongtaek) {
    methods.push({ id: 'extract', icon: '📊', label: '일일업무보고 자동추출 · 직접입력', html: dayReportBox });
  } else if (extraction) {
    methods.push({ id: 'extract', icon: '📊', label: extraction.label, html: extraction.html });
  }
  if (!isPyeongtaek) methods.push({ id: 'paste', icon: '⌨️', label: '직접 붙여넣기', html: pasteHtml });
...
function loadRowForEdit(idx) {
  ...
  if (currentCenter === 'pyeongtaek') { loadPtUnifiedRowForEdit(row); return; }
...
// ============================================
// 평택시청: 일일업무보고 자동추출 + 직접입력 + 업무유형별 현황을 한 화면으로 통합.
// "일자별 실적"(rowSchema)과 "업무유형별 세부현황"(categorySchema)을 한 표에 같이 두고,
// 날짜 범위를 지정하면 그 기간의 표가 만들어지며, 파일 추출 결과도 이 표에 바로 채워진다.
// 저장도 한 번에(manual-entry-bulk 한 번 호출) 이뤄진다. 날짜 범위 생성/제외 체크박스 시각효과는
// LG전자통합의 lgeTotalDateRange()/lgeTotalToggleRowExclude()를 그대로 재사용한다.
// ============================================
let ptUnifiedDates = [];
let ptUnifiedCols = []; // rowSchema + categorySchema를 합친 컬럼 목록(마지막으로 표를 그릴 때 기준)

function renderPyeongtaekEntryPanel() {
  const today = localDateStr(new Date());
  return '<div class="panel" style="max-width:100%;margin-bottom:16px;">'
    + '<h3>일일업무보고 한글파일(HWPX)에서 자동 추출 (기본 입력 방식)</h3>'
    + '<p style="font-size:13px;color:#a1a1a6;margin-bottom:4px;">일일업무보고(.hwpx)를 첨부하면 됩니다 — 여러 날짜 파일을 한 번에 선택할 수 있고, 파일마다 제목의 날짜를 자동으로 인식해 아래 표에 채워줍니다. 날짜를 미리 입력할 필요 없이 파일만 선택하면 됩니다.</p>'
    + '<p style="font-size:12px;color:#5b6472;margin-bottom:8px;">📎 아래 칸을 클릭해 파일을 선택하거나, 파일을 이 카드 위로 끌어다 놓아도(드래그앤드롭) 됩니다.</p>'
    ...
    + '<h3>일자별 실적 + 업무유형별 세부현황 (직접 입력 · 수정)</h3>'
    + '<p style="font-size:13px;color:#a1a1a6;margin:10px 0 6px;">파일을 추출하면 이 표에 자동으로 채워집니다. 파일 없이 날짜 범위를 직접 지정해 입력할 수도 있습니다.</p>'
    + '<div class="entry-row"><label>시작일</label><input type="date" id="ptUnifiedStart" value="' + today + '"></div>'
    + '<div class="entry-row"><label>종료일</label><input type="date" id="ptUnifiedEnd" value="' + today + '"></div>'
    + '<button class="btn-secondary" onclick="generatePtUnifiedRows()">기간 적용</button>'
    + '<button class="btn-primary" id="ptUnifiedSaveBtn" style="display:none;background:#34c759;" onclick="savePyeongtaekDailyReportAll()">전체저장</button>'
    + '<div class="status-msg" id="ptUnifiedStatus"></div>'
    + '<div id="ptUnifiedTemplateArea" style="margin-top:16px;"></div>'
    + '</div>';
}
...
async function generatePtUnifiedRows(forceIncludeDate) {
  ...
  if (!start || !end) { statusEl.className = 'status-msg err'; statusEl.textContent = '시작일과 종료일을 입력해 주세요.'; return; }
  if (start > end) { statusEl.className = 'status-msg err'; statusEl.textContent = '시작일이 종료일보다 늦을 수 없습니다.'; return; }
  if (!rowSchema || rowSchema.length === 0 || !categorySchema || categorySchema.length === 0) {
    statusEl.className = 'status-msg err'; statusEl.textContent = '입력양식이 아직 로드되지 않았습니다. 새로고침 후 다시 시도해 주세요.'; return;
  }
  ...
  const bodyRows = ptUnifiedDates.map(function(date, ri) {
    const cells = ptUnifiedCols.map(function(c) {
      return '<td><input type="text" class="pt-unified-input" data-row="' + ri + '" data-key="' + c.key + '" value="' + carry[c.key] + '" style="width:64px;padding:4px;border:1px solid #2c2c2e;border-radius:4px;font-size:12px;text-align:center;"></td>';
    }).join('');
    return '<tr><td style="position:sticky;left:0;background:#1d1d1f;text-align:center;"><input type="checkbox" class="pt-unified-exclude" data-row="' + ri + '" title="체크하면 저장 시 이 날짜를 제외합니다" onchange="lgeTotalToggleRowExclude(this)"></td>'
      + '<td style="position:sticky;left:30px;background:#1d1d1f;font-weight:600;">' + date + '</td>' + cells + '</tr>';
  }).join('');

  const bulkCells = ptUnifiedCols.map(function(c) {
    return '<td><input type="text" class="pt-unified-bulk-input" data-key="' + c.key + '" placeholder="값" style="width:60px;padding:3px;border:1px solid #2c2c2e;border-radius:4px;font-size:11px;text-align:center;background:#111113;color:#f5f5f7;">'
      + '<button class="btn-outline" style="display:block;width:100%;margin-top:3px;padding:2px 0;font-size:9px;" onclick="applyPtUnifiedColumn(\'' + c.key + '\')">전체반영</button></td>';
  }).join('');
  ...
  area.innerHTML = '<div class="table-scroll"><table><thead>'
    + '<tr><th rowspan="2" style="position:sticky;left:0;background:#111113;font-size:11px;">제외</th><th rowspan="2" style="position:sticky;left:30px;background:#111113;">날짜</th>' + groupHeaderCells + '</tr>'
    + '<tr>' + headCells + '</tr>'
    + '</thead><tbody>' + bulkRow + bodyRows + '</tbody></table></div>'
    + '<p style="font-size:12px;color:#86868b;margin-top:6px;">' + ptUnifiedDates.length + '개 날짜 · 일괄입력 행에 값을 넣고 항목별 전체반영을 누르면 모든 날짜에 채워집니다 · "제외" 열을 체크하면 그 날짜는 저장에서 빠집니다</p>'
    + '<div style="margin-top:10px;"><button class="btn-primary" style="padding:6px 14px;font-size:12px;" onclick="applyPtUnifiedAllColumns()">⚡ 전체 항목 전체반영</button></div>';

  document.getElementById('ptUnifiedSaveBtn').style.display = 'inline-block';
  statusEl.className = 'status-msg ok';
  statusEl.textContent = seed ? (seed.report_date + ' 저장값을 기본으로 재직/투입인원을 채웠습니다.') : '직전 저장 데이터가 없어 빈 값으로 시작합니다.';
}

function applyPtUnifiedColumn(key) {
  const statusEl = document.getElementById('ptUnifiedStatus');
  const bulkInp = document.querySelector('.pt-unified-bulk-input[data-key="' + key + '"]');
  const value = bulkInp ? bulkInp.value : '';
  if (value === '') { statusEl.className = 'status-msg err'; statusEl.textContent = '일괄입력 행에 적용할 값을 먼저 입력해 주세요.'; return; }
  for (let ri = 0; ri < ptUnifiedDates.length; ri++) {
    const inp = document.querySelector('.pt-unified-input[data-row="' + ri + '"][data-key="' + key + '"]');
    if (inp) inp.value = value;
  }
  statusEl.className = 'status-msg ok';
  statusEl.textContent = ptUnifiedDates.length + '개 날짜에 값을 적용했습니다.';
}

function applyPtUnifiedAllColumns() {
  const statusEl = document.getElementById('ptUnifiedStatus');
  let count = 0;
  ptUnifiedCols.forEach(function(c) {
    const inp = document.querySelector('.pt-unified-bulk-input[data-key="' + c.key + '"]');
    if (inp && inp.value !== '') { applyPtUnifiedColumn(c.key); count++; }
  });
  if (count === 0) { statusEl.className = 'status-msg err'; statusEl.textContent = '일괄입력 행에 적용할 값을 하나 이상 입력해 주세요.'; return; }
  statusEl.className = 'status-msg ok';
  statusEl.textContent = count + '개 항목을 ' + ptUnifiedDates.length + '개 날짜 전체에 반영했습니다.';
}

// 저장: rowSchema 항목은 그대로, categorySchema 항목은 "합계"(첫 항목) 대비 비중(%)을 계산해
// "{key}_비중" 키로 함께 저장한다.
async function savePtUnifiedRows() {
  const statusEl = document.getElementById('ptUnifiedStatus');
  const token = centerTokenMap[currentCenter];
  const totalKey = categorySchema[0] ? categorySchema[0].key : null;
  const entries = [];
  ptUnifiedDates.forEach(function(date, ri) {
    const excludeCb = document.querySelector('.pt-unified-exclude[data-row="' + ri + '"]');
    if (excludeCb && excludeCb.checked) return;
    const values = {};
    rowSchema.forEach(function(c) {
      const inp = document.querySelector('.pt-unified-input[data-row="' + ri + '"][data-key="' + c.key + '"]');
      if (inp && inp.value !== '') values[c.key] = { value: inp.value, group: c.group };
    });
    let total = 0;
    if (totalKey) {
      const totalInp = document.querySelector('.pt-unified-input[data-row="' + ri + '"][data-key="' + totalKey + '"]');
      // 천단위 콤마(예: "1,348")가 섞여 있으면 parseFloat이 콤마에서 멈춰버려(예: 1) 비중이 완전히 틀어지므로 먼저 제거한다
      total = parseFloat(String((totalInp && totalInp.value) || '0').replace(/,/g, '')) || 0;
    }
    categorySchema.forEach(function(c, ci) {
      ...
      if (ci !== 0) {
        const n = parseFloat(String(inp.value || '0').replace(/,/g, '')) || 0;
...
// "저장된 데이터 조회·수정" 패널의 "수정하기" — 통합 입력표에 그 날짜 하나만 불러온다.
async function loadPtUnifiedRowForEdit(row) {
  ...
  ptUnifiedCols.forEach(function(col) {
    ...
    const inp = document.querySelector('.pt-unified-input[data-row="' + ri + '"][data-key="' + col.key + '"]');
    if (inp) inp.value = val;
  });
  const area = document.getElementById('ptUnifiedTemplateArea');
  if (area) area.scrollIntoView({ behavior: 'smooth', block: 'center' });
  alert(row.report_date + ' 데이터를 통합 입력표에 불러왔습니다. 값을 수정한 뒤 "전체저장"을 눌러주세요.');
}
