...
## 1. 한 줄 요약
`admin.html`(단일 정적 HTML/JS 파일) → Supabase Edge Function `center-report-upload`(action 기반 단일 함수) → Supabase Postgres.
...
두 URL은 **같은 `admin.html` 파일**을 쓰지만 **별도 Vercel 배포**입니다. 하나만 업데이트하면 다른 쪽은 구버전이 남습니다.
...
- 외부 라이브러리: Chart.js(그래프), SheetJS/XLSX(엑셀 파싱), 둘 다 CDN(jsdelivr, cdnjs) 로드 + jsdelivr 실패시 unpkg 폴백.
