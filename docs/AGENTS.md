...
> 이 문서는 **kkangbi-report** (실적/KPI 관리 도구, `admin.html`+`style.css`+`app.js` + Supabase Edge Function `center-report-upload`)를 다루는 모든 AI 에이전트가 작업 시작 전 반드시 읽어야 합니다.
...
- **전체 교체 파일 원칙**: 패치(diff) 조각이 아니라, 수정이 끝난 파일 **전체**를 항상 다시 내려줍니다. (2026-07-17부터) 프론트엔드는 `admin.html`/`style.css`/`app.js` 3개로 분리되어 있으므로, **수정한 파일만** 전체로 내려주면 됩니다(예: JS 로직만 고쳤으면 `app.js`만, CSS만 고쳤으면 `style.css`만 — 안 건드린 파일까지 매번 재전달할 필요 없음). 백엔드는 `index.ts` 전체 파일을 그대로 유지.
...
이 저장소는 `npm run sync:manuals` 같은 자동 동기화 스크립트가 **없습니다** (정적 파일 3개 + Edge Function 1개 구조라 빌드 파이프라인 자체가 없음). 동기화는 에이전트가 수동으로 위 3가지를 직접 갱신하는 방식으로 대신합니다. `skills/kkangbi-docs-sync/SKILL.md`에 체크리스트가 있습니다.
...
- **파일 구조**: 이 저장소는 `index.html` + `api/*.js` (Next.js API 라우트) 구조가 아니라, **`admin.html`(HTML 뼈대) + `style.css`(전체 스타일) + `app.js`(전체 로직, 2026-07-17 분리) + Supabase Edge Function 1개(`center-report-upload`, action 쿼리파라미터로 라우팅)** 구조입니다. 새 기능을 "새 API 파일"로 만들지 말고 `index.ts` 안에 `if (action === '...')` 블록으로, 새 프론트엔드 로직은 `app.js` 안에 함수로 추가합니다. **토큰 절감을 위해 도입한 구조이므로, 다시 `admin.html`에 인라인으로 합치지 않습니다.**
- **배포**: `admin.html` + `style.css` + `app.js` **3개 파일 세트**를 **두 곳**에 각각(같은 폴더에 함께) 올려야 합니다 — `kkangbi-report.vercel.app`(센터담당자용), `report.xn--2l0b841ao7b.kr`(관리자용, 도메인 자동인증). 한쪽만 올리거나 3개 중 일부만 올리면 화면이 깨지거나 구버전이 남습니다(과거 "한쪽만 배포" 실제 발생 사례 있음 — `CHANGELOG.md` 참고).
