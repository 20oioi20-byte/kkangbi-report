...
console.log('10. 회차 · 제목 · 붙여넣기 · 전월값 손입력');
{
  ...
  // (다) 붙여넣기 — 엑셀(탭) · 메모장(이름 : 값) 둘 다 읽는다
  const xls = parsePasted('인입호\t31,385\t28,204\n응대호\t31,125\t28,142\n\n포기호\t260\t62');
  eq(xls.length, 3, '엑셀에서 복사한 세 줄을 읽는다');
  eq(xls[0], { name: '인입호', val: '28,204' }, '맨 끝 칸을 값으로 본다(당월 열)');
  const txt = parsePasted('인입호 : 28,204건\n응대율：99.8%\n1일 평균 상담건수   1,407건\n빈 줄 아님');
  eq(txt.length, 3, '«이름 : 값» 과 «이름   값» 을 읽고, 짝이 없는 줄은 버린다');
  eq(txt[1], { name: '응대율', val: '99.8%' }, '전각 콜론(：)도 읽는다');
  eq(txt[2].name, '1일 평균 상담건수', '이름에 공백이 있어도 끊지 않는다');
  eq(parsePasted('').length, 0, '빈 글자는 아무것도 안 준다');
  eq(parsePasted('그냥 한 줄').length, 0, '값이 없으면 안 가져온다');
